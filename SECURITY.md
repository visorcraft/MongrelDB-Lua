...
This document describes the security properties of the MongrelDB Lua client
and how to report vulnerabilities.

## Overview

The MongrelDB Lua client is a LuaRock that talks to `mongreldb-server` over
...
- Server and network errors are mapped to typed error objects
  (`auth`, `not_found`, `constraint`, `connection`, `query`), not leaked as
  bare strings.
...
The MongrelDB Lua client depends on LuaSocket at runtime. LuaSocket is MIT
licensed and widely audited. The vendored JSON encoder has no further
dependencies. Report dependency vulnerabilities through GitHub's Dependabot
...
- the MongrelDB Lua client version, Lua version, and OS,
